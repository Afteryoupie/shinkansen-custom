...
$('translate-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    const force = (mode === 'retranslate');
    await browser.tabs.sendMessage(tab.id, { type: 'TRANSLATE_PRESET', payload: { slot, force } });
    closePopup();
  } catch (err) {
...
$('edit-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (resp?.ok) {
      ...
      statusEl.style.color = resp.editing ? '#D74B4B' : '#86868b';
    }
  } catch {
...
if ($('restore-btn')) {
  $('restore-btn').addEventListener('click', async () => {
    ...
    try {
      const [tab] = await browser.tabs.query({ active: true, currentWindow: true });
      if (!tab?.id) { btn.disabled = false; return; }
      statusEl.textContent = t('popup.status.restoring');
